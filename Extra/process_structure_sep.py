...
def process_structure_sep(data, i, slab_dir, bulk_dir):
    slab_data = []

    crystal_id = data['index'][i]
    mp_id = data['mp_id'][i]

    POSCAR = Poscar.from_file(os.path.join(r"D:\Temp_Data\surface_model\CIF_MP\bulk_opt_all", f"{crystal_id}"))
    ...
    for j, slab in enumerate(unique_slabs):
        ...
        slab_info = [f"{crystal_id}_{j}", mp_id, fomula, crystal_id, index, shift, num_atom, 0, 'na']
        ...
        # # generate thicker slab if ouc has 6 or less atom
        # if len(ouc) <= 6:
        #     ouc_thickness = get_ouc_thickness(ouc)
        #
        #     slab_generator = SlabGenerator(structure, miller_index,
        #                                    min_slab_size=10 + ouc_thickness,
        #                                    min_vacuum_size=15.0, center_slab=True, primitive=False)
        #     slabs = slab_generator.get_slabs()
        #     for k, slab in enumerate(slabs):
        #         if shift == slab.shift:
        #             num_atom = len(slab.sites)
        #
        #             slab_filename = os.path.join(slab_dir, f"{crystal_id}_{j}_{1}")
        #             # slab.to(filename=slab_filename, fmt='poscar')
        #             with open(slab_filename, 'w') as f:
        #                 p = fix_middle_atom(slab)
        #                 f.write(str(p))
        #
        #             slab_info = [f"{crystal_id}_{j}_{1}", mp_id, fomula, crystal_id, index, shift, num_atom, 1, 'na']
        #             slab_data.append(slab_info)
        #
        #     slab_generator = SlabGenerator(structure, miller_index,
        #                                    min_slab_size=10 + 2 * ouc_thickness,
        #                                    min_vacuum_size=15.0, center_slab=True, primitive=False)
        #
        #     slabs = slab_generator.get_slabs()
        #     for k, slab in enumerate(slabs):
        ...
        #             num_atom = len(slab.sites)
        #
        #             slab_filename = os.path.join(slab_dir, f"{crystal_id}_{j}_{2}")
        #             # slab.to(filename=slab_filename, fmt='poscar')
        #             with open(slab_filename, 'w') as f:
        #                 p = fix_middle_atom(slab)
        #                 f.write(str(p))
        #
        #             slab_info = [f"{crystal_id}_{j}_{2}", mp_id, fomula, crystal_id, index, shift, num_atom, 1, 'na']
        #             slab_data.append(slab_info)
